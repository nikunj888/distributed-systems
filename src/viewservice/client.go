...
func (ck *Clerk) Ping(viewnum uint) (View, error) {
	...
	//log.Printf("Pinging viewserver with %v\n", args);
	ok := call(ck.server, "ViewServer.Ping", args, &reply)
	//log.Printf("Received reply %v from viewserver\n", reply);
	if ok == false {
...
// get the current view of the system from viewservice
...
// get the primary from viewservice
